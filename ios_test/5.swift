...
// 프로토콜의 확장(Extension)은 기존에 정의된 프로토콜에 추가적인 기능을 구현하는 개념아다.
// 이를 통해 프로토콜을 채택한 모든 타입에 대해 기본 구현을 제공하거나, 
// 특정 메서드나 속성을 추가하여 확장할 수 있다.
...
// 프로토콜 지향 프로그래밍은 객체 간의 상호작용을 프로토콜로 정의하고, 
// 프로토콜을 통해 코드의 재사용성과 유연성을 높이는 프로그래밍 패러다임 이다. 
// 이는 특히 구조체와 프로토콜의 조합을 통해 다형성을 구현하는 Swift에서 강력한 기법이다.
...
// Swift에서 함수가 1급 객체로 취급된다는 의미는 함수가 변수에 저장되거나, 
// 파라미터로 전달되거나, 반환값으로 사용될 수 있다는 것이다. 
// 이로 인해 함수형 프로그래밍 패러다임을 적용할 수 있고, 고차 함수 등을 사용할 수 있다.
...
// 콜백 함수는 특정 이벤트나 작업이 완료된 후 호출되는 함수이다.
// 주로 비동기 작업이나 완료 후 추가 작업이 필요한 경우에 사용되며, 함수의 파라미터로 전달돤다.
...
// 고차함수 사용한 코드
var newResult = numbersArray.filter { $0 % 2 != 0 }.map { $0 * $0 }.reduce(0, +)
...
// 함수형 프로그래밍은 상태 변이 없이 함수를 통해 로직을 구성하는 프로그래밍 패러다임이다.
// 주로 순수 함수와 불변성을 강조하여, 코드의 예측 가능성과 재사용성을 높이고, 
// 병렬 처리에 유리하게 설계된다.
