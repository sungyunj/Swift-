...
계산 속성은 실질적인 메서드(함수)
단지, 외부에서 겉모습을 속성처럼 보이게 해서, 코드를 직관적으로 사용할 수 있는 것 뿐
따라서, 어떤 속성에 대해 읽기 메서드 / 쓰기 메서드를 구현하는 것에 불과함
(두개의 함수/메서드를 구현한다고 생각하고 접근하면 쉽게 구현 가능)


/**==================================================================
 [9번 문제]
  - 클래스(또는 구조체)에서 속성 감시자의 의미가 뭔가?
  - 아래의 Profile 클래스에서 message의 변수(속성)에 didSet 속성감시자를 구현.
    (didSet에서 아래의 메세지를 출력.)
...
// 속성감시자는 "특정" 저장 속성에 딸려있는 메서드에 불과
// 저장 속성의 값이 변했을때, 항상 자동으로 호출되는 메서드 !
// 따라서, 당연히 구현방식은 메서드와 동일
...
/**==================================================================
 [13번 문제]
  - Singleton패턴이 무엇? 아는 대로 간단하게 서술하고, (자유롭게) 간단한 예제 코드를
    작성해 보세요.
 ===================================================================**/
...
/**==================================================================
 [14번 문제]
  - 클래스의 타입 메서드의 구현에서, static 키워드와 class 키워드의 차이는 뭔가?
 ===================================================================**/


"타입" 메서드로 구현하면, 기본적으로 static(고정) 키워드를 붙여야 함.
static - 상속해서 재정의 불가
class - 상속해서 재정의 가능
...
값타입은 원칙적으로 (메서드 실행시) 새로운 스택프레임을 만들어 인스턴스 값자체를 복사해서 사용하는 것임 (깊은 복사 일어남)
따라서, 값타입의 메서드의 실행시의 동작은 (mutating이 안 붙어 있다면)
복사한 새로운 데이터 값을 만들어 그 새로운 값을 변경시키는 것임 (원본 변경이 안됨)

결국 mutating이란 것은, (값타입이지만) 실질적으로 원본 데이터를 변경시키는 것을 표기위한 목적인 것임
...
/**==================================================================
 [16번 문제]
  - 아래에서, SomeSuperclass를 상속한 SomeSubclass에서.. 상위 클래스의
    aValue속성을 계산속성으로 자유롭게 재정의.
    (계산 속성으로 재정의시 내부 구현은 자유.)
 ===================================================================**/
